...
def H(redshift, H0 = None, Omega_M = None, Omega_R = None):
    ...
    #Note that for large E1 this works for Hubble as a function of temperature, H(T/T0) but not strictly when used as function of redshift as would then need to account for gstarS degrees of freedom 
    tempArray = [70000., 330000., 17144880.,100.e6, 160.e6, 2.e9,10.e9]
    gstarEArray = [3.36, 10.6, 10.6, 18.00, 62.49, 82.50, 86.22]
    gstarE0 = 3.36

    logTempArray = np.log(tempArray)
    logGstarEArray = np.log(gstarEArray)

    def gstarE(temp):
        return np.exp(np.interp(np.log(temp),logTempArray,logGstarEArray))

    T0 = 2.34866e-4 
    
    def gstarEz(redshift):
        return gstarE(redshift * T0)
    ...
    return H0 * np.sqrt(gstarEz(redshift)*( redshift**(3.) * Omega_M + redshift**(4.) * Omega_R + (1-Omega_R-Omega_M) )/gstarE0)
...
#def dmwel_conversion( redshift ):
    u"""Returns :math:`\\left(z+1\\right)^3`

    Returns conversion factor in front of the z and energy integrals
    which takes the expansion of comoving volume into account.
    
    For dmwel do not need to divide by $H(redshift)$ because that appears in the numerator of
    the net emission rate and so cancels.
...
def f_function(transfer_functions_log10E, log10E, z_inj, z_dep, normalization,
    ...
    how_to_integrate = DarkOptions.get('E_integration_scheme','dmwel')
   
    if how_to_integrate not in ['logE','energy','dmwel']: #inserted new DMwEL option
        ...
        #modify to exclue DMwEL:
    if len(E) == 1 and how_to_integrate !='dmwel': how_to_integrate = 'energy' # Handling of a dirac-spectrum is inside the integration part w.r.t energy

    #AJW Next norm_by introduced on 14 September 2019
    norm_by = DarkOptions.get('normalize_spectrum_by','unity')
    ##insert new dmwel_conversion     
    if norm_by == 'unity':
        norm =  normalization
    else:
        norm = ( conversion(z_dep,alpha=alpha) )*( normalization )
    
    #print("how_to_integrate = ",how_to_integrate,"norm_by = ",norm_by)
    ...
    #include new DMwEL sub-routine in energy_integral routine
    #print(spec_phot.shape)
    energy_integral = np.zeros( shape=(len(z_dep),len(z_inj)), dtype=np.float64)
    Enj = (transfer_functions_log10E)
    for i in xrange(len(energy_integral)):
        ...
        elif how_to_integrate == 'dmwel':
            # AJW recall made energy_sequence global in creating model.dmwel_model
            #print('--->HERE')
            for k in xrange(i,len(energy_integral[i])):
                if not need_to_interpolate:
                    int_phot = transfer_phot[i,:,k]*spec_phot[:,k]*(E[:]**1)
                    ...
                    int_phot = evaluate_transfer(Enj,transfer_phot[i,:,k],E)*spec_phot[:,k]*(E[:]**1)
                    int_elec = evaluate_transfer(Enj,transfer_elec[i,:,k],E)*spec_elec[:,k]*(E[:]**1)
                #assert np.shape(int_phot)==np.shape(int_elec)
                energy_integral[i][k] = np.sum( int_phot + int_elec)
                ###print("energy_integral[",i,"][",k,"] \n",energy_integral[i][k])
    ...
    for i in xrange(len(z_integral)):
        low = max(i,0)
        #low = i
        ##AJW modify for dmwel - to ensure z integral doesn't get divided by H(z) (on 21 July 2019).
        #if how_to_integrate == 'dmwel':
         #   integrand = energy_integral[i,low:]
        #else:
